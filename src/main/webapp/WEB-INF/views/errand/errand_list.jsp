...
<section class="page-header
        <c:choose>
            <c:when test="${sessionScope.loginSess.role eq 'ERRANDER'}">
                header-errander
            </c:when>
            <c:otherwise>
                header-user
                    ...
                    <div class="task-image">
                        <c:choose>
                            <c:when test="${fn:startsWith(e.displayImageUrl, 'http')}">
                                <img src="${e.displayImageUrl}" class="img-cover" alt="심부름 이미지">
                            </c:when>
                            <c:otherwise>
                                <img src="${pageContext.request.contextPath}${e.displayImageUrl}"
